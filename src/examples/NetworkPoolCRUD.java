 ...
 * 
 * For Portgroup-backed network pool: Verify that one or more port groups are
 * available in vSphere. The port groups must be available on each ESX/ESXi host
 * in the cluster, and each port group must use only a single VLAN. Port groups
 * with VLAN trunking are not supported. For Isolation-backed network pool:
 * Verify that a vSphere distributed switch is available. For Vlan-backed
 * network pool: Verify that a range of VLAN IDs and a vSphere distributed
 * switch are available in vSphere. The VLAN IDs must be valid IDs that are
 * configured in the physical switch to which the ESX/ESXi servers are
 * connected.
 * 
...
public class NetworkPoolCRUD {
	...
	public static void usage() {
		System.out
				.println("java NetworkPoolCRUD Vcloud user@organization password VimServerName PortGroupName dvSwitchName NetworkPoolName CertificateKeyStorePath[optional] CertificateKeyStorePassword[optional]");
		System.out
		...
		System.out
				.println("java NetworkPoolCRUD https://vcloud username@Organization password vimservername portgroupname dvswitchname networkpoolname certificatekeystorepath certificatekeystorepassword");
		System.exit(0);
	}
	...
	private static String getDVSwitchMoref(String dvSwitchName,
		...
		Expression dvSwitchExpression = new Expression(QueryDvSwitchField.NAME,
				dvSwitchName, ExpressionType.EQUALS);
		Expression vcNameExpression = new Expression(QueryDvSwitchField.VCNAME,
				vimServerName, ExpressionType.EQUALS);
	...
	public static void main(String args[]) throws VCloudException,
		...
		VlanPoolType vlanPoolUpdate = updateNetworkPool(vimServerRef,
				dvSwitchMoref, args[6]);
