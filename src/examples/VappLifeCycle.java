...
/**
 *
 * VappLifeCycle sample: A Structured REST Workflow Example, This sample uploads an ovf
 * package into vcloud as a vapptemplate.
 *
 * @author Ecosystem Engineering
 */

public class VappLifeCycle {
	...
	public static Vapp newvAppFromTemplate(ReferenceType vAppTemplateReference,
		...
		instVappTemplParams.setName("VappLifeCyclevApp");
	...
	public static CatalogItemType createNewCatalogItem(
		...
		catalogItem.setName("VappLifeCycle-VappTemplate");
		catalogItem.setName("VappLifeCycle-VappTemplate");
	...
	/**
	 * VappLifeCycle Program Usage
	 */
	public static void getUsage() {
		System.out
				.println("java VappLifeCycle vCloudURL user@vcloud-organization password orgName vdcName ovfFileLocation catalogName");
		System.out
				.println("java VappLifeCycle https://vcloud user@vcloud-organization password orgName vdcName ovfFileLocation catalogName");
		System.exit(0);
	}

	/**
	 * VappLifeCycle: A Structured REST Workflow Example
	 *
	 * @param args
	...
	public static void main(String args[]) throws VCloudException,
		...
		// Sample usage
		if (args.length < 7)
			VappLifeCycle.getUsage();

		// Logging in and Getting the Organization List
		VappLifeCycle.login(args[0], args[1], args[2]);

		// Finding a vdc
		Vdc vdc = VappLifeCycle.findVdc(args[3], args[4]);
		...
		VappTemplate VappLifeCycleTemplate = vdc.uploadVappTemplate(
				"VappLifeCycle-VappTemplate", "VappLifeCycle-VappTemplate",
				args[5], false, vdc.getVdcStorageProfileRefs().iterator().next()
				,catalog.getReference());
		System.out.println("Uploaded vAppTemplate - "
				+ VappLifeCycleTemplate.getReference().getName());
		System.out.println("-------------------------------");

		// Instantiating the vAppTemplate
		Vapp vapp = VappLifeCycle.newvAppFromTemplate(VappLifeCycleTemplate
				.getReference(), vdc);
