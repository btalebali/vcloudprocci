...
public class SelfProvisioningSample {
	...
	public static void main(String[] args) throws TimeoutException,
		...
		try {
			...
			if (args.length == 8) {
				System.setProperty("javax.net.ssl.trustStore", args[6]);
				System.setProperty("javax.net.ssl.trustStorePassword", args[7]);
	...
	private static void addIsolatedOrgVdcNetwork(AdminOrganization adminOrg)
		...
		try {
			AdminOrgVdcNetwork orgVdcNet = adminVdc
					.createOrgVdcNetwork(OrgVdcNetworkParams);
			if (orgVdcNet.getTasks().size() > 0) {
				orgVdcNet.getTasks().get(0).waitForTask(0);
			}
			System.out.println("	Isolated Org vDC Network : "
					+ orgVdcNet.getResource().getName() + " created - "
	...
	private static void addNatRoutedOrgVdcNetwork(AdminOrganization adminOrg)
		...
		try {
			...
			System.out.println("	Nat-Routed Org vDC Network : "
					+ orgVdcNet.getResource().getName() + " created - "
					+ orgVdcNet.getResource().getHref());
		} catch (VCloudException e) {
			System.out.println("FAILED: creating org vdc network - "
	...
	private static void addBridgedOrgVdcNetwork(AdminOrganization adminOrg)
		...
		try {
			...
			System.out.println("	Bridged Org vDC Network : "
					+ orgVdcNet.getResource().getName() + " created - "
					+ orgVdcNet.getResource().getHref());
		} catch (VCloudException e) {
			System.out.println("FAILED: creating org vdc network - "
	...
	public static void usage() {
		System.out
				.println("java SelfProvisioningSample vCloudURL user@organization password ProviderVdcName ExternalNetworkName NetworkPoolName CertificateKeyStorePath[optional] CertificateKeyStorePassword[optional]");
		System.out
		...
		System.out
				.println("java SelfProvisioningSample https://vcloud user@System password providervdcname externalnetworkname networkpoolname certificatekeystorepath certificatekeystorepassword");
		System.exit(0);
	}
}